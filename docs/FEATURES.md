...
| 個股篩選 | 第一根表態 | 量價突破訊號 |
| 狀態篩選 | 有 新高/強勢/三線 | 至少其中一個徽章 |
| 狀態篩選 | 新近 新高/強勢/三線 ≤ N 日 | streak 在 1~N 天 |
| 成交量 | 成交量(張) >= | 最新日 |
| 成交量 | 量比(今/5日均) >= | 流動性放大 |
...
欄位：☆ / 代碼 / 名稱 / **新高** / **強勢** / **三線** / 產業 / 收盤 / 漲跌% / 成交量(張) / **5日均量** / **20日均量** / 外資N日 / 投信N日 / 自營N日 / 法人合計 / 融資餘額 / 融資增減 / 融券餘額 / 融券增減
...
**表格各欄位定義**（2026-09-23 記錄）：

CB 端：
- **CB量比** = 當日 CB 成交量 ÷ 過去 60 個交易日均量(不含今天)；均量 <5 張顯示「—」(基期太小倍
  數沒意義，如新掛牌 CB)。算法見 `daily_cb_scan.py` 的 `base = sum(vl[n-61:n-1]) / min(60, n-1)`。

個股技術面：
- **股當日/5日/20日** = 個股收盤價對 1/5/20 個交易日前的累積漲跌%。
- **股量比** = 個股當日成交量 ÷ 20 日均量(不含今天)；≤0.5 視為量縮。
- **位階** = (今日收盤 − 60日最低) ÷ (60日最高 − 60日最低) × 100%。
- **均線** = 收盤>月線(20MA)>季線(60MA) → 「多頭」；收盤<月線 → 「月線下」；其餘 → 「整理」。

個股籌碼面：
- **外資5日/投信5日** = 最近 5 個交易日買賣超合計(張)。
- **大戶4週** = 集保股權分散表「持股 100 張以上」佔股本比例，本期 − 4 期(週)前的差(百分點)。
- **融資日增/融券日增** = 融資(或融券)餘額今日 − 昨日(張)。
- **融資5日** = (今日融資餘額 ÷ 5 日前餘額 − 1) × 100%。
- **券資比** = 融券餘額 ÷ 融資餘額 × 100%。
- **週報分** = 借用 §2.6 週報同一套 12 項加權評分，這裡僅供參考(不受多頭門檻篩選)。
- **券商買超前2/前5集中** = 當日券商分點買超(買張−賣張)排名前 2 名；前 5 名合計淨買 ÷ 當日
  個股總成交量 × 100%，算法見 `daily_cb_scan.py` 的 `broker_today()`。
...
| `scripts/lib/cb_alerts.py` | Phase 6 CB 快訊:掃 `cbDailyTrading` / `cbInstitutional`,挑出 ①CB 大量(量≥200張且≥3×20日均量) ②價漲量增(漲≥3%且量≥2×**5日**均量、≥100張) ③個股法人單日大買(**CB 對應個股**=代號前4碼,查 `cbInstitutional` 股數/1000 轉張,單一法人≥500張,且該股任一 CB 當日量≥100張),組一則獨立 Telegram 訊息。門檻是檔案頂端常數。送到專用 bot(`TG_CB_BOT_TOKEN`+`TG_CB_CHAT_ID`,兩個 secret),沒設就 fallback 用 pipeline 摘要那支 `TG_BOT_TOKEN`/`TG_CHAT_ID`。本機驗:`cd scripts && python -m lib.cb_alerts`(加 `--send` 真的送)。只在該輪有重解 CB 日交易時才觸發(margin-late 等 `--only-sources` 跑法會跳過) |
| `scripts/parse_etf.py` | ETF 持股 |
| `scripts/vcp_scanner.py` | VCP 選股 |
